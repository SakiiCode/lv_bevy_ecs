use lv_codegen::{CodeGen, LvWidget, Rusty};
...
fn main() {
    ...
    writeln!(file, "{}", code_formatted).unwrap();

    let widgets_path = out_path.join("widgets.rs");
    let widgets_macros: Vec<TokenStream> = codegen
        .get_widgets()
        .iter()
        .flat_map(LvWidget::gen_impl)
        .collect();
    let widgets_code = quote! {
        #(#widgets_macros)*
    };
    let mut widgets_file = File::create(&widgets_path).unwrap();
    let macros_formatted = prettyplease::unparse(
